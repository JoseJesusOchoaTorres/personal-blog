...
El primero *“sass-dev”* estará escuchando nuestro `index.scss` y cada que hagamos un cambio en algún archivo `.scss` este será transpilando y copiado todo nuestro código dentro del folder de `build` en un archivo llamado `index.css`
...
- #### 3.2.- Tamaño definido manualmente
    ...
    **Punto de ruptura / Breakpoint**
    ...
    Los breakpoints son un punto de cambio para nuestro layout, y usamos estos para modificar la distribución del contenido dependiendo la anchura de cada dispositivo, ya que definitivamente no queremos mostrar de la misma manera el contenido en un smartphone, tablet o un monitor con resoluciones superiores. Esto terminará de quedar claro más adelante.
    ...
    **SASS Mixins**
    ...
    Ahora bien, ¿cómo terminan de encajar todo esto con nuestro grid system? La respuesta es simple; suponiendo que queremos mostrar 3 elementos en la misma línea con un grid system de 12 columnas:
    ...
    Todo el tiempo estos elementos tendrán la misma distribución, pero si queremos cambiar el tamaño y distribución de cada columna dependiendo el breakpoint usamos clases como estas:
    ...
    Y para cada breakpoint sería lo mismo con al diferencia de que cambiamos un poco la clase agregando el breakpoint:
    ...
    Una vez terminado esto tendrás un grid system funcional, pero recordemos uno de los principios del desarrollo de software, [*DRY*](https://www.wikiwand.com/es/No_te_repitas) _(Don't Repeat Yourself)_.
    ...
    Y esto sin lugar a duda sería una versión más limpia, ordenada y mantenible. Esto tendría que seguir generando todas nuestras clases del grid system tal y como las definimos anteriormente.
